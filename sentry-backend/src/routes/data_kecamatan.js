...
router.post("/bulk-delete", auth, requireRole("staff"), async (req, res) => {
  try {
    ...
    if (normalizedIds.length === 0) {
      return res.status(400).json({ message: "ids wajib berupa array kecamatan_id yang valid" });
    }

    const deletedCount = await DataKecamatan.destroy({
      where: { kecamatan_id: { [Op.in]: normalizedIds } },
    });

    return res.json({
      message: `${deletedCount} data kecamatan berhasil dihapus`,
